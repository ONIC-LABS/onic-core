...
import { BaseRunner, OnicRunnerLLMInput, OnicRunnerToolInput } from "@/agents/onic/runners/base.js";
import type {
  OnicAgentRunIteration,
  OnicAgentTemplates,
  OnicParserInput,
  OnicRunInput,
...
export class DefaultRunner extends BaseRunner {
  ...
  async llm({ signal, meta, emitter }: OnicRunnerLLMInput): Promise<OnicAgentRunIteration> {
  ...
  async tool({ state, signal, meta, emitter }: OnicRunnerToolInput) {
  ...
  protected async initMemory({ prompt }: OnicRunInput): Promise<BaseMemory> {
    ...
    const prevConversation = [...history.messages, this.renderers.user.message({ prompt })]
      ...
      .map((message) => {
        if (message.role === Role.USER) {
          ...
          const text = isEmpty
            ? (this.templates?.userEmpty ?? OnicUserEmptyPrompt).render({})
            : (this.templates?.user ?? OnicUserPrompt).render({
                input: message.text,
                meta: {
                  ...message?.meta,
  ...
  @Cache({ enumerable: false })
  get templates(): OnicAgentTemplates {
    const customTemplates = this.input.templates ?? {};

    return {
      system: customTemplates.system ?? OnicSystemPrompt,
