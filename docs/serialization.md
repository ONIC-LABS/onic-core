...
> [!TIP]
>
> Location within the framework `onic-agent-framework/serializer`.
...
```ts
import { Serializer } from "onic-agent-framework/serializer/serializer";
...
const instance = new MyClass("onic");
...
```ts
import { Serializable } from "onic-agent-framework/internals/serializable";
