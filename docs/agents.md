...
## Implementation in Onic Agent Framework
...
Our Onic Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new onicAgent({
  llm: new OllamaChatLLM(), // for more explore 'onic-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'onic-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'onic-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/onic_advanced.ts) to see how to modify them.
