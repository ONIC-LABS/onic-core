...
> Location within the framework `onic-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "onic-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "onic-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello onic!" });
```
...
import { BaseLLM } from "onic-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/onic.ts).
