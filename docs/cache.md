...
> [!TIP]
>
> Location within the framework `onic-agent-framework/cache`.
...
```ts
import { UnconstrainedCache } from "onic-agent-framework/cache/unconstrainedCache";
...
```ts
import { SlidingCache } from "onic-agent-framework/cache/slidingCache";
import { WikipediaTool } from "onic-agent-framework/tools/search/wikipedia";
...
import { BaseMessage } from "onic-agent-framework/llms/primitives/message";
...
```ts
import { FileCache } from "onic-agent-framework/cache/fileCache";
import * as os from "node:os";

const cache = new FileCache({
  fullPath: `${os.tmpdir()}/onic_file_cache_${Date.now()}.json`,
});
...
const fileCache = await FileCache.fromProvider(memoryCache, {
  fullPath: `${os.tmpdir()}/onic_file_cache.json`,
});
...
```ts
import { Cache } from "onic-agent-framework/cache/decoratorCache";
...
```ts
import { Cache, SingletonCacheKeyFn } from "onic-agent-framework/cache/decoratorCache";
...
```ts
import { CacheFn } from "onic-agent-framework/cache/decoratorCache";
...
```ts
import { BaseCache } from "onic-agent-framework/cache/base";
import { NotImplementedError } from "onic-agent-framework/errors";
